...
const openWeatherKey = '8996470d6bf290ebc138cd7b6247e156'; // not good practice, we will handle this later
const baseUrl = 'https://api.openweathermap.org/data/2.5';
const cacheCounter = 10; // in minutes

function getWeather(position) {
	var weatherURL = `${baseUrl}/weather?APPID=${openWeatherKey}`

	//  check if chache should be used
	if (getDataFromCache(localGet('weather_ts'))) {
...
function getForecast(position) {
	var forecastURL = `${baseUrl}/forecast?APPID=${openWeatherKey}`;

	// Check if cache should be used
	if (getDataFromCache(localGet('forecast_ts'))) {
...
function getDataFromCache(timestamp) {
	if (timestamp) {
		var savedTime = Date.parse(timestamp);
		var now = new Date().getTime();
		return (Math.floor(((now - savedTime) / 1000) / 60) < cacheCounter);
	} else {
		return false;
	}
}
